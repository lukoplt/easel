...
/// then verify consistency. Environment-gated: runs only when PACHECK_TEST_MSAPP points at
...
public sealed class RenameE2ETests
{
    ...
    {
        var msapp = Environment.GetEnvironmentVariable("PACHECK_TEST_MSAPP");
        if (string.IsNullOrWhiteSpace(msapp) || !File.Exists(msapp) || !PacRunner.Detect().Found)
        {
            _out.WriteLine("skipped: set PACHECK_TEST_MSAPP to a real .msapp and install pac to run this E2E.");
            return;
        }

        var pac = PacRunner.Create();
        var work = Path.Combine(Path.GetTempPath(), "pacheck-e2e-" + Guid.NewGuid().ToString("n"));
