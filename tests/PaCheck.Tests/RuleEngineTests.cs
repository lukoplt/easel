...
public sealed class RuleEngineTests
{
    ...
    private IReadOnlyList<Finding> RunRules(PaCheckConfig? cfg = null)
    {
        var analysis = AppAnalysis.FromFolder(TestPaths.SampleApp);
        var engine = RuleEngine.CreateDefault();
        return engine.Run(analysis, cfg ?? PaCheckConfig.Empty);
    }
    ...
    private static PaCheckConfig ConfigLoaderInline(string yaml)
    {
