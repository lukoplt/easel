...
public sealed class ConfigAndBaselineTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "pacheck-cfg-" + Guid.NewGuid().ToString("n"));
        var deep = Path.Combine(root, "a", "b", "c");
        Directory.CreateDirectory(deep);
        try
        {
            File.WriteAllText(Path.Combine(root, ".pacheck.yml"), "rules:\n  unused-media: off\n");
    ...
    public void Baseline_round_trips_and_suppresses_known_findings()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), "pacheck-baseline-" + Guid.NewGuid().ToString("n") + ".json");
        try
        {
