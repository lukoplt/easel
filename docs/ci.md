...
jobs:
  lint:
    ...
    steps:
      ...
      - name: Run pacheck
        uses: pacheck/pacheck/action@v0     # or a pinned SHA
        with:
          path: ./src/MyApp          # unpacked pa.yaml folder or a .msapp
          format: sarif
          fail-on: warning
          output: pacheck.sarif

      - name: Upload SARIF
        if: always()
        uses: github/codeql-action/upload-sarif@v3
        with:
          sarif_file: pacheck.sarif
```

The action installs `pac` and `pacheck`, runs `pacheck lint`, and fails the job when
...
dotnet tool install --global PaCheck.Tool
export PATH="$PATH:$HOME/.dotnet/tools"

pacheck lint ./src/MyApp --format sarif -o pacheck.sarif --fail-on warning
```
...
comment. The comment body is pacheck's own generated Markdown — no PR title/body is
interpolated into any `run:` step, so there is no injection surface.

```yaml
# .github/workflows/pacheck-diff.yml
name: pacheck diff
on: [pull_request]
...
jobs:
  diff:
    ...
    steps:
      ...
      - run: dotnet tool install --global PaCheck.Tool
      - run: echo "$HOME/.dotnet/tools" >> "$GITHUB_PATH"

      - name: Generate diff
        run: pacheck diff base/src/MyApp head/src/MyApp --format markdown -o diff.md
