# pacheck — Architektura a plán vývoje
...
> Pracovní název `pacheck` — finální název dořešit před prvním releasem (kolize na NuGet/GitHub, vyslovitelnost).
...
pacheck/
├── src/
│   ├── PaCheck.Cli/            # entrypoint, definice příkazů, DI kompozice
│   ├── PaCheck.Core/           # doménový model, loader, symbol table, dep graph
│   ├── PaCheck.Fx/             # wrapper nad Microsoft.PowerFx (parse, cache, AST walk)
│   ├── PaCheck.Pac/            # PacRunner — detekce, verze, unpack/pack, temp správa
│   ├── PaCheck.Rules/          # lint pravidla (IRule implementace) + rule engine
│   ├── PaCheck.Analysis/       # analyze, stats, secrets, diff, rename engines
│   └── PaCheck.Output/         # renderery: Console, JSON, SARIF, HTML
├── tests/
│   ├── PaCheck.Tests/          # unit + snapshot testy
│   └── fixtures/               # sada testovacích aplikací (unpacked YAML + msapp)
├── action/                     # GitHub Action (composite: install pac → run pacheck)
├── docs/                       # dokumentace, pravidla, příklady configů
└── .pacheck.yml                # ukázkový config
```
...
Model, symbol table i graf se staví **jednou** a sdílí mezi příkazy — kombinace `pacheck lint --with-stats` nesmí parsovat dvakrát.

## 5. Klíčové komponenty

### 5.1 PacRunner (PaCheck.Pac)
...
- Temp správa: `%TEMP%/pacheck/<hash vstupu>/`, úklid po doběhu, `--keep-temp` pro debugging.
...
- Baseline: `pacheck lint --write-baseline` → `.pacheck-baseline.json` (fingerprint nálezů); následné běhy reportují jen nové nálezy.
- Potlačení: config (`ignore` per pravidlo/cesta/element).

### 5.6 DiffEngine (PaCheck.Analysis)
...
### 5.10 Konfigurace
`.pacheck.yml` v kořeni repa (hledá se směrem nahoru):
```yaml
rules:
  naming-convention:
    severity: warning
...
### 5.11 Doctor
`pacheck doctor`: verze nástroje, pac (nalezen/verze/min. verze), PowerFx verze, podporovaná verze pa.yaml schématu, zápis do temp, výsledek testovacího unpacku (volitelně).
...
- [ ] **T0.7 (S)** `pacheck doctor` v1 (nástroj, pac, PowerFx, schéma, temp).
...
- [ ] **T2.2 (M)** Config loader `.pacheck.yml`: hledání nahoru, validace, defaulty, `ignore` glob.
...
- [ ] **T2.10 (M)** GitHub Action (composite: install pac → install pacheck → run → upload SARIF) + ukázkový workflow do docs.
