...
Write-Host "Installing pac + pacheck..."
...
if ([string]::IsNullOrWhiteSpace($version)) {
  dotnet tool install --global PaCheck.Tool 2>$null; if ($LASTEXITCODE -ne 0) { dotnet tool update --global PaCheck.Tool }
} else {
  dotnet tool install --global PaCheck.Tool --version $version 2>$null; if ($LASTEXITCODE -ne 0) { dotnet tool update --global PaCheck.Tool --version $version }
}

Write-Host "Running pacheck lint..."
pacheck lint "$path" --format $format --fail-on $failOn --output "$output"
$code = $LASTEXITCODE

# Surface SARIF as a build artifact when produced.
if (($format -eq 'sarif') -and (Test-Path $output)) {
  Write-Host "##vso[artifact.upload containerfolder=pacheck;artifactname=pacheck-sarif]$output"
}

# exit 1 = findings over threshold -> fail the task; 2/3/4 = error -> fail.
if ($code -ne 0) {
  Write-Host "##vso[task.complete result=Failed;]pacheck exit $code"
  exit $code
}
