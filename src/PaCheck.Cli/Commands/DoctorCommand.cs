...
/// <summary>`pacheck doctor` — environment diagnostics.</summary>
public sealed class DoctorCommand
{
    public int Run()
    {
        ...
        table.AddRow("pacheck", "[green]ok[/]", $"{ToolInfo.ToolName} {ToolInfo.Version}");
        ...
        try
        {
            var probe = Path.Combine(Path.GetTempPath(), "pacheck", "doctor-" + Guid.NewGuid().ToString("n"));
